...
fig_percentage_distribution = px.box(
    filtered_percentage,
    y='DISCOUNT PERCENTAGE',
    title='<b>Discount Percentage Distribution</b>',
    color_discrete_sequence=['#F5DF1F'],
)
fig_percentage_distribution.update_layout(
    yaxis=dict(title='Discount Percentage', range=[0, 75]),
)

right_column.plotly_chart(fig_percentage_distribution, use_container_width=True)

#---Discount Price distribution---#
fig_disc_distribution = px.box(
    filtered_percentage,
    y='DISCOUNT PRICE',
    title='<b>Discount Price Distribution</b>',
    color_discrete_sequence=['#F5DF1F'],
)
fig_disc_distribution.update_layout(
    yaxis=dict(title='Discount Price', range=[0, 400000]),
)

#---True Price distribution---#
fig_true_distribution = px.box(
    filtered_percentage,
    y='TRUE PRICE',
    title='<b>True Price Distribution</b>',
    color_discrete_sequence=['#3279BD'],
)
fig_true_distribution.update_layout(
    yaxis=dict(title='True Price', range=[0, 500000]),
)

left_column, right_column = st.columns(2)
left_column.plotly_chart(fig_disc_distribution, use_container_width=True)
right_column.plotly_chart(fig_true_distribution, use_container_width=True)
