...
fig_rating_hist = px.histogram(
    filtered_percentage,
    x='RATING',
    title='<b>Rating Histogram</b>',
    color_discrete_sequence=['#F5DF1F'],
    nbins=100,
)
fig_rating_hist.update_layout(
    xaxis=dict(title='Rating', range=[0, 5.1]),
    yaxis=dict(range=[0, 260]),
    bargap=0.2,
)
right_column.plotly_chart(fig_rating_hist, use_container_width=True)

#---Discount Price histogram---#
fig_disc_hist = px.histogram(
    filtered_percentage,
    x='DISCOUNT PRICE',
    title='<b>Discount Price Histogram</b>',
    color_discrete_sequence=['#3279BD'],
    nbins=50,
)
fig_disc_hist.update_layout(
    xaxis=dict(title='Discount Price', range=[0, 400001]),
    yaxis=dict(range=[0, 200]),
    bargap=0.2,
)
#---Discount Percentage histogram---#
fig_percentage_hist = px.histogram(
    filtered_percentage,
    x='DISCOUNT PERCENTAGE',
    title='<b>Discount Percentage Histogram</b>',
    color_discrete_sequence=['#3279BD'],
    nbins=50,
)
fig_percentage_hist.update_layout(
...
left_column, right_column = st.columns(2)
left_column.plotly_chart(fig_disc_hist, use_container_width=True)
right_column.plotly_chart(fig_percentage_hist, use_container_width=True)
